...
PROMPTS = {
    ...
    "docs_generate_issues": (
        ...
        "A repository has already been checked out for you. Examine the existing code and then think through, "
        "step by step, which documentation tasks are needed to properly write documentations.\n"
        "Generate documentation tasks in a logical order that builds user understanding.\n"
        "IMPORTANT: Focus on user needs and avoid technical jargon unless necessary. The documentation should be "
        ...
        "2. For each significant folder in the codebase, create a documentation task with the path as the title (e.g., 'path: /src/core')\n"
        ...
        "   - A clear, specific title (including the [path] or [file])\n"
    ...
    "docs_validate_issues": (
        ...
        "Carefully consider each task and determine if it:\n"
        "1. Makes sense and is essential for user understanding\n"
        "2. Is not overly technical or complex\n"
        "3. Does not overlap with other tasks\n"
        "4. Is arranged in a logical learning sequence\n"
        "If any problems are found, provide an updated list of documentation tasks that are valid.\n"
        "If none of the problems above are found, use the 'approve_issues' tool to approve the tasks. "
        "Do not generate new tasks if the only improvements are minor wording changes.\n"
    ),
    "docs_decompose_feature": (
        "You are creating documentation sections and subsections as part of a plan to document the following tasks:\n"
        "{issues}\n"
        "A repository has already been checked out for you. Examine the existing code and then think through, "
        ...
        "2. A detailed description of the section and its purpose\n"
        "3. Specific documentation requirements and quality criteria\n\n"
        "Guidelines for task section breakdown:\n"
        "- Each task section should represent a major functional component or feature\n"
        "- Task sections should be organized in a logical sequence of implementation\n"
        "- Include setup and configuration tasks where relevant\n"
        "- Include integration tasks with other components\n"
        "- Include error handling and edge cases\n"
        "- Include performance considerations where applicable\n"
        "- Include security considerations where relevant\n"
        "- Every task section must be clear and actionable\n"
        "- Consider dependencies between tasks\n\n"
        "IMPORTANT: Focus on breaking down the feature into clear, implementable tasks. Each task section should "
        "represent a distinct piece of functionality that can be implemented independently. Do not include tasks "
        "that are not directly related to the feature. Do not include functionality beyond the scope of the section.\n"
        "IMPORTANT: Make sure each task section is distinct and doesn't overlap with other tasks.\n"
    ),
    ...
    "docs_regenerate_subtasks": (
        "A documentation section has been broken down into subsections but some were deemed invalid.\n"
        "Your task is to regenerate the subsections based on the feedback provided.\n"
        "The documentation has been broken down into the following main sections:\n"
        "{issues}\n"
        "Original Subsections: {subtasks}\n"
        "Failed Subsections: {auditedSubtasks}\n"
        "Feedback: {feedbacks}\n"
        "A repository has already been checked out for you. Examine the existing code and then think through, "
        "step by step, which subsections are needed to fully document this section.\n"
        "For each subsection, you must provide:\n"
        "1. A clear, specific title\n"
        "2. A detailed description of the content to be covered\n"
        "3. Specific documentation requirements and quality criteria\n\n"
        "Guidelines for documentation breakdown:\n"
        "- Each subsection should focus on a single topic or concept\n"
        "- Subsections should build knowledge progressively\n"
        "- Include practical examples and use cases\n"
        "- Consider different user personas and their needs\n"
        "- Include necessary prerequisites and dependencies\n"
        "- Add troubleshooting and common issues sections where relevant\n"
        "- Include code examples where appropriate\n"
        "- Consider accessibility and readability\n"
        "- Include visual aids where helpful\n"
        "- Every subsection must be clear and actionable for users\n"
        "- Consider edge cases and error scenarios\n\n"
    ),
    "docs_dependency_tasks": (
        "Review the following target documentation subsection and determine if it depends on any other subsections.\n"
        "Target subsection: {target_task}\n"
        "Other subsections:\n{subtasks}\n\n"
        "Dependencies should always be one way, marking which subsections a subsection depends on.\n"
        "If any dependencies are found, link the uuid of the target subsection "
        "with a list of uuids for any subsections it depends on.\n"
        "IMPORTANT: do not create circular dependencies.\n"
    ),
    "docs_generate_system_prompts": (
        ...
        "[High-level description of the documentation goals and target audience]\n\n"
        "AI Role and Expertise:\n"
